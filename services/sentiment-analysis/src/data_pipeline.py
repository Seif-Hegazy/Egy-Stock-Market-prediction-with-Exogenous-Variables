...
import nltk
import pandas as pd
from sector_map import SECTOR_MAP, TICKER_TO_SECTOR, TICKER_ALIASES
...
RSS_FEEDS = [
    # === Egyptian Financial News (Primary) ===
    ...
    "https://economyplusme.com/en/feed/",
    
    # === International - Egypt Coverage ===
    "https://www.investing.com/rss/news_1.rss",  # General
    "https://www.investing.com/rss/news_285.rss",  # Egypt-specific
    "https://www.zawya.com/en/economy/north-africa/rss",
    "https://www.reuters.com/places/egypt/feed",
    "https://www.bloomberg.com/feeds/bpol/sitemap_news.xml",
    "https://www.ft.com/rss/world/mideast",
    
    # === Regional Financial News ===
    "https://www.al-monitor.com/rss",
    "https://www.arabnews.com/cat/4/rss.xml",  # Economy
    "https://www.thenationalnews.com/rss/business/xml",
    "https://www.middleeasteye.net/news/rss",
    "https://english.aawsat.com/rss/latest",
    
    # === Market Data & Analysis ===
    "https://www.tradingeconomics.com/egypt/rss-feed",
    "https://www.worldbank.org/en/country/egypt/rss",
    "https://www.imf.org/en/Countries/EGY/rss",
    
    # === Sector-Specific ===
    "https://www.forbes.com/africa/rss/",
    "https://www.cnbc.com/id/100727362/device/rss/rss.html",  # Middle East
    "https://www.arabfinance.com/en/rss",
    
    # === Emerging Markets Focus ===
    "https://www.emergingmarkets.org/rss/",
    "https://frontierinvestor.com/feed/",
    
    # === Additional Egyptian Sources ===
    "https://www.egx.com.eg/english/rss.aspx",  # EGX official
    "https://enterprise.press/feed/",
    "https://see.news/feed/",
]
...
DIRECT_SCRAPE_SOURCES = {
    ...
    "Enterprise Press": {
        "url": "https://enterprise.press/stories/",
        "selector": "h2.entry-title a",
        "base": ""
    },
    "Mada Masr (English)": {
        "url": "https://www.madamasr.com/en/section/economy/",
        "selector": "h3.post-title a",
        "base": ""
    },
    "Al-Ahram Weekly": {
        "url": "https://weekly.ahram.org.eg/News/Economics.aspx",
        "selector": "div.article-title a",
        "base": "https://weekly.ahram.org.eg"
    },
    "CBE Announcements": {
        "url": "https://www.cbe.org.eg/en/EconomicResearch/Publications/Pages/default.aspx",
        "selector": "div.ms-listlink a",
        "base": "https://www.cbe.org.eg"
    },
    "Ministry of Finance": {
        "url": "https://mof.gov.eg/en/media-center/news",
        "selector": "div.news-item a",
        "base": "https://mof.gov.eg"
    },
    "MSCI Egypt": {
        "url": "https://www.msci.com/emerging-markets",
        "selector": "div.article-title a",
        "base": "https://www.msci.com"
    },
    "S&P Dow Jones - Egypt": {
        "url": "https://www.spglobal.com/spdji/en/indices/equity/sp-egx-30/",
        "selector": "article a",
        "base": "https://www.spglobal.com"
    }
}
...
def identify_tickers(text):
    """Identify tickers mentioned in text using aliases"""
    found_tickers = set()
...
def aggregate_sentiment_features(jsonl_file):
    ...
    - sector_sentiment: Avg sentiment of articles mentioning the Sector or other tickers in sector
    """
    print("\n📊 Aggregating Sector News Signals...")
    ...
    # Identify tickers for each article
    df['tickers'] = df['text'].apply(identify_tickers)
    
    # Create daily features for each ticker
    daily_features = []
    ...
    for date in dates:
        ...
        for ticker in all_tickers:
            ...
            # 2. Sector Sentiment
            # Articles mentioning Sector Name OR other tickers in same sector
            # BUT NOT the target ticker (to avoid double counting direct signal)
            
            # Identify articles mentioning other tickers in sector
            sector_tickers = SECTOR_MAP.get(sector, [])
            other_tickers = [t for t in sector_tickers if t != ticker]
            
            def has_sector_signal(row_tickers, text):
                # Check for other tickers
                if any(t in row_tickers for t in other_tickers):
                    return True
                # Check for Sector Name explicitly
                if sector.lower() in text.lower():
                    return True
                return False
                
            sector_mask = day_articles.apply(
                lambda row: has_sector_signal(row['tickers'], row['text']) and (ticker not in row['tickers']), 
                axis=1
            )
    ...
    # Save to CSV (simulating Postgres save for now)
    output_path = '/opt/airflow/data/processed/daily_sentiment_features.csv'
    features_df.to_csv(output_path, index=False)
    print(f"✓ Saved {len(features_df)} stock-day rows to {output_path}")
    
    # TODO: Add Postgres saving logic here
    # engine = create_engine(os.getenv("POSTGRES_CONNECTION_STRING"))
    # features_df.to_sql('sentiment_features', engine, if_exists='append', index=False)
    
    return features_df

def build_training_dataset(entries):
    """Process entries and save to JSONL with deduplication"""
    # Updated for unified directory structure
    # Use relative path to support both local and Docker execution
    output_file = '/opt/airflow/data/raw/news/articles.jsonl'
...
def main():
    ...
    aggregate_sentiment_features(output_file)
